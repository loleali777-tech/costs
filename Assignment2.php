...
//  دالة تطبع "مرحبًا بك يا طالب"
function welcome Student() {
    echo "مرحبًا ياطالب<br>";
}

// دالة تستقبل اسمًا وتطبع رسالة ترحيب.
function greet($name) {
    echo "مرحباً يا $name!<br>";
}

//  دالة تستقبل رقمين وتعيد ناتج ضربهما.
...
//  دالة تستقبل العمر وتعيد "بالغ" أو "طفل".
function checkAge($age) {
    return ($age >= 18) ? "بالغ" : "طفل";
}

//  دالة تستقبل عددًا غير محدود من الأرقام وتعيد أكبر رقم.
function maxNumber(...$numbers) {
    return max($numbers);
}

//  دالة تستقبل نصًا وتعيد عدد الكلمات فيه.
function wordCount($text) {
    return str_word_count($text);
}

//  دالة مجهولة (Anonymous) لطباعة "مرحبًا".
$anonymousHello = function() {
    echo "مرحبًا<br>";
};

//  دالة Callback لتنفيذ رسالة "تم الانتهاء" بعد مهمة ما.
function performTask($callback) {
    echo "جار تنفيذ المهمة...<br>";
    $callback();
}

//  استخدام array_filter لتصفية المنتجات ذات السعر الأكبر من 100.
...
//  دالة Higher-Order ترجع دالة تطبق نسبة خصم على سعر
...
//  Currying Function: تأخذ نسبة ضريبة، ثم السعر، وتعيد السعر شامل الضريبة
...
// 1 Lambda Function لحساب مربع رقم
$square = fn($n) => $n * $n;

//  Higher-Order Function تستقبل مصفوفة أرقام ودالة وتُعيد مصفوفة مُعدَّلة
...
//  Function Composition لدمج دالتين: double ثم subtractFive
function double($n) {
    return $n * 2;
}
